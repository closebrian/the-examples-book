...
**Motivation:** We will pause in our series of `pandas` and `numpy` projects to learn one of the most important parts of writing programs -- functions! Functions allow us to reuse snippets of code effectively. Functions are a great way to reduce the repetition of code, also a great way to keep your code organized and readable. 

**Context:** Focusing on learning how to write functions in Python.
...
Can you confirm that sorting the ksuids automatically sorts the observations by date & time?
...
In this dataset `latitude` and `longitude` values are given to us in degrees. We want to convert the degrees to radians. 

Write a function called `degrees_to_radians` that accepts a latitude or longitude value in degrees, and returns the same value in radians.
...
Write a function that accepts two `pandas` series that contains a  `latitude` and `longitude` value, and also returns the distance between the two points in Kilometers. Call this function `get_distance`.
...
[TIP]
====
In the formula above, the latitude and longitudes values need to be converted from degrees to radians. Your function from the Question 3 will be perfect for this!
...
Write a function called `plot_stations`. The `plot_stations` should accept a dataset as an argument and produce a plot with the station's locations plotted on a map.

For consistancy we will use `plotly` to make the plot. https://stackoverflow.com/questions/53233228/plot-latitude-longitude-from-csv-in-python-3-6[This] stackoverflow post will show some samples. For further understanding https://plotly.com/python-api-reference/generated/plotly.express.scatter_geo.html[here] is the explanation for the function.
